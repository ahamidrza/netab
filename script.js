...
window.onload = function () {
    ...
    document.getElementById('search_input').placeholder = 'Search what do you want from ' + search_engine + ' ...';
    document.getElementById(search_engine + '_search_engine_picker').classList.add('picked_search_engine');
    for (let i = 0; i < document.getElementsByClassName('search_engine_picker_button').length; i++) {
        document.getElementsByClassName('search_engine_picker_button')[i].onclick = function () {
            document.getElementById(search_engine + '_search_engine_picker').classList.remove('picked_search_engine');
            localStorage.setItem('search_engine', this.getElementsByTagName('span')[0].innerHTML);
            search_engine = localStorage.getItem('search_engine');
            this.classList.add('picked_search_engine');
            document.getElementById('search_input').placeholder = 'Search what do you want from ' + search_engine + ' ...';
        }
    }
    document.getElementById('search_input').onkeydown = function () {
        if (event.key === 'Enter') {
            if (search_engine === 'yandex') {
                window.open('https://yandex.com/search/text=' + this.value, '_self')
            } else if (search_engine === 'bing') {
                window.open('https://www.bing.com/search?q=' + this.value, '_self')
    ...
    document.getElementById('time').innerHTML = new Date().getHours() + ':' + new Date().getMinutes() + ' ' + (new Date().getHours() >= 12 ? 'PM' : 'AM');
    setInterval(function () {
        if (document.getElementById('time').innerHTML !== new Date().getHours() + ':' + new Date().getMinutes() + ' ' + (new Date().getHours() >= 12 ? 'PM' : 'AM')) {
            new Date().getHours() + ':' + new Date().getMinutes() + ' ' + (new Date().getHours() >= 12 ? 'PM' : 'AM')
        }
    }, 2000);
    const apiKey = "6057dd1ebb7435800b8ba3d53207e421";
    ...
    fetch(url)
        ...
        .then(data => {
            document.getElementById('location').innerHTML = inputVal;
            document.getElementById('weather').innerHTML = Math.floor(data.main.temp * (9 / 5) + 32) + '°F';
            document.getElementById('weather_icon').src = data.weather[0].icon + '.png';
        })
        .catch(() => {
